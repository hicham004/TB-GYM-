    ...
    providers: [
      provideRouter([]),
      {
        ...
        useValue: {
          ...
          resendInvitation: vi.fn(() => of(invitation({ sendCount: 2 }))),
...
describe('Invitations', () => {
  ...
  it('resends a pending invitation and shows the new send count', async () => {
    const { fixture, host, api } = await render({
    ...
    expect(api.resendInvitation).toHaveBeenCalledWith('invitation-1');
    expect(host.textContent).toContain('Sent 2 time(s)');
    expect(host.textContent).toContain('A new invitation link was queued.');
  });
  ...
  it('revokes a pending invitation and withdraws the actions with it', async () => {
    ...
    expect(api.revokeInvitation).toHaveBeenCalledWith('invitation-1');
