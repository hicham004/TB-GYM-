...
and client profiles under multiple coaches. **Proposed:** an email match links through a
verified invitation; it must never expose one coach's profile to another coach.
...
client may correct fields they are explicitly allowed to supply. After acceptance, the plan
says profile corrections are coach-only; the exact field ownership matrix must be approved.
...
1. Is a tenant always one coach, or an organization with owner, multiple coaches, staff, and
   transferable clients?
2. Can one client use the same login with multiple coaches? The architecture recommends yes.
3. Which onboarding fields can a client edit after invitation acceptance, and which are
   coach-only? Can a client correct legal/contact details?
4. Does one subscription always bind training and diet to one period, or can each service
   have independent dates? Can an active subscription have no training program?
5. Are installments supported? Exactly which payment standings grant full access, and does
   an overdue client retain historical content?
6. What is the tenant time zone, week start, and behavior when a program starts midweek or a
   coach changes dates after logs exist?
7. Which BMR formula, activity/TDEE model, unit conventions, and sex/formula options are
   approved? Who is responsible for clinical review?
8. Is macro energy strictly 4/4/9, provider calories, or both with discrepancy reporting?
9. Which RPE/RIR reference table and progression model is the first supported version? What
   plate increments and rounding policy apply?
10. Can clients truly edit accessory prescriptions, or only log actuals/request a change?
11. What counts as diet completion and a missed workout for experience changes?
12. Which countries launch first? This determines privacy, health-data, messaging consent,
    tax/currency, retention, and payment requirements.
13. Do you own or license all uploaded/imported recipes, videos, generated art, and any themed
